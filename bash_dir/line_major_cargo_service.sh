...
xls_path="${XL_IDP_PATH_IMPORT}/lines_${XL_IMPORT_TERMINAL}/major_cargo_service/"
...
do
  ...
  if [ $? -eq 0 ]
	...
	exit_message=$(python3 ${XL_IDP_ROOT}/scripts/major_cargo_service.py "${csv_name}" "${json_path}" 2>&1 > /dev/null)
