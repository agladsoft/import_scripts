#!/bin/bash
echo ${XL_IDP_ROOT}
xls_path="${XL_IDP_ROOT}/reference/reference_report_on_order/"
...
do
  ...
  if [ $? -eq 0 ]
	...
	python3 /mnt/xl_idp_root/scripts_for_bash_with_inheritance/reference_report_on_order.py "${csv_name}" "${json_path}"
