...
xls_path="${XL_IDP_PATH_IMPORT}/lines_${XL_IMPORT_TERMINAL}/reel_shipping_tracking/"
...
do
  ...
  if [ $? -eq 0 ]
	...
	exit_message=$(python3 ${XL_IDP_ROOT}/scripts/reel_shipping.py "${csv_name}" "${json_path}" 2>&1 > /dev/null)
