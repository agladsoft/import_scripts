...
xls_path="${XL_IDP_PATH_IMPORT}/lines_${XL_IMPORT_TERMINAL}/one/"
#xls_path=/home/timur/Anton_project/import_xls-master/lines_nutep/one
...
do
  ...
  if [ $? -eq 0 ]
	...
	exit_message=$(python3 ${XL_IDP_ROOT}/scripts_for_bash_with_inheritance/one.py "${csv_name}" "${json_path}" 2>&1 > /dev/null)
