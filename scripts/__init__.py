...
DICT_HEADERS_COLUMN_ENG: Dict[Tuple, str] = {
    ("№ п/п", "№", "№ пп."):
        "number_pp",
    ("Статус", "Тип", "Type"):
        "container_size_and_type",
    ("Тип",):
        "container_type",
    ("Размер",):
        "container_size",
    ("Номер контейнера", "№контейнера", "№ контейнера", "Container", "Номер Контейнера"):
        "container_number",
    ("Пломба", "№ пломбы", "№пломбы"):
        "container_seal",
    (
        "Вес брутто (кг)", "Вес груза", "Вес груза брт,кг", "Вес груза брт, кг", "Weight",
        "Вес, бр. груза", "Вес, бр. груза, кг", "Вес груза брткг", "Вес груза брт.кг"
    ):
        "goods_weight_with_package",
    (
        "Количество мест", "Кол-во мест", "мест", "Кол-во", "Кол-во мест", "Places", "К-во мест", "К-во мест/вес товара",
        "Кол-о мест", "Кол -во мест"
    ):
        "package_number",
    (
        ...
        "goods_name",
    ("ТНВЭД", "КОД ТНВЭД", "Код Тн ВЭД", "Российский Код ТНВЭД (10 знаков)", "Код ТНВЭД"):
        "tnved",
    ("Отправитель", "Грузоотправитель", "Shipper To Order", "Грузоотправитель наименование"):
        "shipper_name",
    (
        "Страна отправителя", "Страна грузоотправителя", "Страна отправления", "Страна", "Страна порта погрузки",
        "Страна затарки", "Страна погрузки", "Страна грузоотправителя", "Отправитель Страна",
        "Страна  грузоотправителя", "Грузоотправитель адрес"
    ):
        "tracking_country",
    ("Получатель", "Грузополучатель", "Consignee Seal", "Грузополуча тель", "Грузополучатель наименование"):
        "consignee_name",
...
DICT_CONTENT_BEFORE_TABLE: Dict[Tuple, str] = {
    ("ДАТА ПРИХОДА", "дата прихода", "Дата прихода", "Дата подхода", "Port of Loading", "Договор с портом ",
     "Дата прибытия", "дата прибытия"): "shipment_date",
...
LIST_LINE_CITY: List[str] = ['admiral', 'akkon_lines', 'c_star', 'sidra', 'safetrans', 'ucak_line', 'verim_uneti',
                             'login', 'econship', 'major_cargo_service','lider_line','fesco','m_line','eccl','siv_mena_line']
